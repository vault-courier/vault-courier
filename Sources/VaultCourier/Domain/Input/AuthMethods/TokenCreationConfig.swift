...
/// Configuration for creating a Vault token
public struct TokenCreationConfig: Sendable {
    ...
    /// If specified, the token will be periodic; it will have no maximum TTL (unless an ``TokenCreationConfig/tokenMaxTimeToLive`` is also set) but every renewal will use the given period. Requires a root token or one with the sudo capability.
    public var tokenPeriod: Duration?

    /// Name of the entity alias to associate with during token creation. Only works in combination with ``VaultCourier/TokenCreationConfig/roleName`` property. If this has been specified, the entity will not be inherited from the parent.
